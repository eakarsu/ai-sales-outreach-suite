const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:5202';
...
const adminCookie = await login('admin@ai-sales-outreach.local', 'admin123');
const managerCookie = await login('manager@ai-sales-outreach.local', 'manager123');
const analystCookie = await login('analyst@ai-sales-outreach.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/contacts', analystCookie);
...
const records = await expectJson('/api/entities/contacts', managerCookie);
const rowId = records.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/contacts/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/contacts/approve`, {
  method: 'POST',
  headers: {
...
console.log('AI Sales Outreach Suite smoke passed');
